...
class Person(Base):
    ...
    # テーブル名
    # →　moduleの名前が定数として代入される
    __tablename__ = "person"

    # カラムの定義
    # →　primary_key=Trueはオートインクリメントが働く、自動採番。ユニーク（唯一）じゃないとだめ
    ...
    # データを辞書型で取得する
    # クラス内で定義する関数をメソッドという（Pythonでメソッドを定義する場合は、第一引数にselfを入れる）
    def getData(self):
        # →　not None→　IDがある時
        if self.id is not None:
            return {
                # →　pythonの一般的な型に変換。カラムをそのまま使えないため、int()やstr()に変換している
                "id":int(self.id),
                "name":str(self.name),
...
# →　SQLAlchemyの接続方法（しないとそもそも使えない、定型文）
# ORMエンジンの使用準備と接続
