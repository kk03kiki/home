...
class Person(Base):
    ...
    # カラムの定義
    # →　(integer整数,データが登録されたとき自動で番号が振り分けられる、１からカウント。primary_key=Trueはセットで覚える)
...
# テストコード
# →　上記にかいたコードのチェック（実際はなくてもいい、動く）
# moduleの名前が定数として代入されている。
# 実行されたら__name__が__main__に代わる（テストコード時入れる）
if __name__ == "__main__":
    # ORMエンジンの使用準備と接続
    # コピペで対応、test.dbが作成される
